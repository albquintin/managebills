...
public interface CashRepository extends JpaRepository<Cash, Long> {
    @Query("SELECT c FROM Cash c WHERE MONTH(c.paymentDate) = :month AND YEAR(c.paymentDate) = YEAR(CURDATE()) AND c.iva10amount > 0 ORDER BY c.paymentDate")
    List<Cash> findCashByMonth(Long month);
    @Query("SELECT c FROM Cash c WHERE MONTH(c.paymentDate) = :month AND YEAR(c.paymentDate) = YEAR(CURDATE()) AND c.iva21amount > 0")
